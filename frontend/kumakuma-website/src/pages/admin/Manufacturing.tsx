      ...
      <div className="max-w-4xl">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Tagline (Atas)</label>
              <input
                type="text"
                value={tagline}
          ...
          <div className="space-y-4">
             <h3 className="text-lg font-semibold text-gray-900">Fitur / Poin Kunci (3 Item)</h3>
             
             {features.map((feature, i) => (
                <div key={i} className="bg-white rounded-xl shadow-sm p-6">
                    <h4 className="font-medium text-gray-800 mb-4">Fitur #{i+1}</h4>
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                        <div>
                             ...
                             <textarea
                                value={feature.title}
                                onChange={(e) => handleFeatureTitleChange(i, e.target.value)}
                                className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500"
                                rows={2}
                             ...
                             <div className="flex items-center gap-4 mb-3">
                                 ...
                                 <div className="flex-1">
                                    <input
                                        type="file"
                                        accept="image/*"
                                        onChange={(e) => handleIconFileChange(i, e.target.files?.[0] || null)}
                                        className="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100"
                                    />
                                    <p className="text-xs text-gray-500 mt-1">Upload gambar/icon baru untuk mengganti.</p>
                                 </div>
                             </div>
                        </div>
                    </div>
