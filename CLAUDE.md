...
- **`middleware/upload.js`** — Multer with `CloudinaryStorage`, **not** disk storage. Files go to the `katiga` folder on Cloudinary; `resource_type` is `raw` for PDFs, `video` for mp4/webm, `image` otherwise. Accepts jpeg/jpg/png/gif/webp/pdf/mp4/webm up to 50 MB. `req.file.path` is an absolute `https://res.cloudinary.com/...` URL.
...
- **`client/src/services/api.ts`** — Single source of truth for all API calls. `VITE_API_URL` is normalised to always end with `/api`. `api.getImageUrl()` passes absolute URLs through and prepends the server origin to legacy relative paths.
...
- New uploads go to Cloudinary and are stored in MongoDB as **absolute** `https://res.cloudinary.com/...` URLs. Older documents may still hold legacy relative paths like `/uploads/{filename}`.
- `api.getImageUrl(path)` handles both: it returns `path` unchanged when it starts with `http`, and otherwise prepends the server origin. Never construct image URLs manually — always use this helper.
- `server.js` still serves `/uploads` as static, which only matters for those legacy paths.
